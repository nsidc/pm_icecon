"""
compute_tb_linreg_rev_psn25.py

Compute linear regressions for day(s) with F17 and AMSR2 data
this version uses amsr2 as X and F17 as Y
"""
...
def compute_linreg(
        ...
        fn_valid_='./bt_doymasks_{hem}/bt_doymask_n_{doy:03d}.dat',
        hem='nh',
):
    ...
    landmask = np.fromfile(
        fn_landmask, dtype=np.uint8).reshape(ydim, xdim)

    # Valid files have value 100 where ice was historically found
    valid = np.fromfile(fn_valid_.format(hem=hem,
                        doy=doy), dtype=np.uint8).reshape(ydim, xdim)

    is_valid_mask = (landmask == 0) & \
                    (valid != 0)
    ...
    for chan in chan_list:
        ...
        tb_old = np.divide(
            np.fromfile(fn_old, dtype=np.int16).reshape(ydim, xdim),
            10.0)

        tb_new = np.divide(
                np.array(ds_new['HDFEOS']['GRIDS'][f'{H}pPolarGrid25km']['Data Fields'][f'SI_25km_{HEM}_{f17_to_au[chan]}']),  # noqa
                10.0
        )
        ...
        lr.fit(tb_new[is_valid].reshape(-1, 1), tb_old[is_valid])
        ...
        # slopes.append(slope)
        # intercepts.append(intrc)

    print('', flush=True)

    """
    slope_arr = np.array(slopes)
    intercept_arr = np.array(intercepts)

    slope_mean = slope_arr.mean()
    slope_std = slope_arr.std()

    intercept_mean = intercept_arr.mean()
    intercept_std = intercept_arr.std()

    print('Slope:')
    print(f'  mean: {slope_mean}')
    print(f'   std: {slope_std}')

    print('Intercept:')
    print(f'  mean: {intercept_mean}')
    print(f'   std: {intercept_std}')
    """
